...
const ParamSlider = ({ id, name, min, max, sliderAction, state, setState }) => {
  ...
  const onSlide = ({target: {value}}) => {
    ...
    AudioNodeGraph.getTarget(id, name).setValueAtTime(transformedValue, AudioNodeGraph.audioCtx.currentTime);
    setState({...state, [name]: transformedValue, [`${name}-slider`]: value})
  }

  const onChange = ({target: {value}}) => {
    AudioNodeGraph.getTarget(id, name).setValueAtTime(value, AudioNodeGraph.audioCtx.currentTime);
